...
from healthgen.generation.models import KVAE_miss
from absl import flags, app

FLAGS=flags.FLAGS
# Uncommented due to duplicate definition. All flags defined in original kvae model.
# # Model
flags.DEFINE_integer('m_dim', 91, 'Dimension of missingness masks.')
...
flags.DEFINE_bool('sample_m', False, 'Whether to sample the reconstructed masks from'
                                     'a Bernoulli distribution or to use the mean.')
# # Training
...
flags.DEFINE_bool('learn_scale', False, 'Whether or not to learn the scale of the reconstruction.')

class KVAEMissGenModel(BaseGenModel):
    def __init__(self, seed, x_dim, m_dim, u_dim, a_dim, z_dim, activation, dropout, dense_x_a,
                 dense_a_x, init_kf_mat, noise_transition, noise_emission, init_cov,
                 K, dim_rnn_alpha, num_rnn_alpha, scale_recon, use_smoothed_a, sample_m, learn_scale):
        # Model parameters
        # General
        self.x_dim = x_dim
        self.m_dim = m_dim
    ...
    def build_model(self):
        model = KVAE_miss(x_dim=self.x_dim, m_dim=self.m_dim, u_dim=self.u_dim, a_dim=self.a_dim, z_dim=self.z_dim,
                      ...
                      use_smoothed_a=self.use_smoothed_a, sample_m=self.sample_m,
                      learn_scale=self.learn_scale).to(self.device)
        return model
    ...
    def build_dataloader(self, X, y, batch_size):
        ...
        assert (self.x_dim + self.m_dim) == data_train.shape[1], F'x_dim specified in model does not match data!' \
                                                  F'Is {(self.x_dim, self.m_dim)}, should be {data_train.shape[1]}!'
        ...
        for epoch in range(1, FLAGS.gen_epochs+1):
            ...
            for train_batch, train_label in train_dataloader:
                ...
                for param in self.model.parameters():
                    param.grad = None
                # Forward pass
                recon_train_batch, m_recon_train_batch = self.model(train_batch, train_label, compute_loss=True)
                # Compute loss
                loss_tot, loss_vae, loss_lgssm, debug_losses = self.model.loss
                # Backward pass
                loss_tot.backward()
                # Update weights
                optimizer.step()
                ...
                train_loss_x_a += debug_losses[0].item()
                train_loss_a_x += debug_losses[1].item()
                train_loss_az_u += debug_losses[2].item()
                train_loss_z_au += debug_losses[3].item()
            ...
            wandb.log({'train_loss_tot': train_loss_tot / train_num,
                       ...
                       'epoch': epoch})
            # Log debugging losses
            wandb.log({'train_loss_x_a': train_loss_x_a / train_num,
                       'train_loss_a_x': train_loss_a_x / train_num,
                       'train_loss_az_u': train_loss_az_u / train_num,
                       'train_loss_z_au': train_loss_z_au / train_num,
                       'epoch': epoch})
            if FLAGS.learn_scale:
                wandb.log({'scale': np.exp(self.model.scale.cpu().detach().numpy()),
                           'epoch': epoch})
            ...
            if epoch % FLAGS.gen_val_freq == 0:
                ...
                with torch.no_grad():
                    for val_batch, val_label in val_dataloader:
                        ...
                        # Forward pass
                        recon_val_batch, m_recon_val_batch = self.model(val_batch, val_label, compute_loss=True)
                        # Compute loss
                        val_loss_tot, loss_vae, loss_lgssm, _ = self.model.loss
    ...
    def reconstruct(self, X, y, N):
        ...
        with torch.no_grad():
            data_recon, m_mean_recon = self.model(data_orig, label_orig, compute_loss=False)
            # Sample from m distribution to get reconstructions of masks
            m_distr = Bernoulli(probs=m_mean_recon)
            m_recon = m_distr.sample().int().cpu().detach().numpy().transpose(0,2,1)



        return data_orig[:,:self.m_dim,:].cpu().detach().numpy(), m_recon