...
class SRNN(nn.Module):
    ...
    def build(self):
        ...
        if len(self.dense_hz_x) == 0:
            ...
            for n in range(len(self.dense_hz_x)):
                if n == 0:
                    # Uncommented to enable conditional generation
                    # dic_layers['linear'+str(n)] = nn.Linear(self.dim_RNN_h+self.z_dim, self.dense_hz_x[n])
                    dic_layers['linear' + str(n)] = nn.Linear(self.dim_RNN_h + self.z_dim + 1, self.dense_hz_x[n])
                else:
                    dic_layers['linear'+str(n)] = nn.Linear(self.dense_hz_x[n-1], self.dense_hz_x[n])
                dic_layers['activation'+str(n)] = self.activation
                dic_layers['dropout'+str(n)] = nn.Dropout(p=self.dropout_p)
        self.mlp_hz_x = nn.Sequential(dic_layers)
        self.gen_mean = nn.Linear(dim_hz_x, self.y_dim)
        self.gen_logvar = nn.Linear(dim_hz_x, self.y_dim)
        # self.gen_m_mean = nn.Sequential(nn.Linear(dim_hz_x, self.y_dim), nn.Sigmoid())
    ...
    def generation_x(self, z, h):
        ...
        y = log_y
        # y = torch.exp(log_y)
        # y_mean_bn = self.gen_m_mean(hz_x)
        # y = y_mean_bn
    ...
    def forward(self, x, labels, compute_loss):
        ...
        z_cat = torch.cat((z, labels), 2)
        y = self.generation_x(z_cat, h)
    ...
    def get_loss(self, x, y, z_mean, z_logvar, z_mean_p, z_logvar_p, seq_len, batch_size, beta=1):
        ...
        loss_recon = torch.square(x - y)
        # Bernoulli LL for the case when we only use masks as input
        # eps = 1e-6
        # y = torch.clamp(y, eps, 1 - eps)
        # loss_recon = -(x * torch.log(y) + (1 - x) * torch.log(1 - y))
        loss_KLD = -0.5 * torch.sum(z_logvar - z_logvar_p 
                - torch.div(z_logvar.exp() + (z_mean - z_mean_p).pow(2), z_logvar_p.exp()))
        
        loss_recon = torch.sum(loss_recon) / (batch_size * seq_len)
    ...
    def generate_synth(self, labels, seq_len, det_gen=False, batch_size=64):
        ...
        # Get list of batch sizes
        # n_full_batches = np.floor(N/batch_size)
        # batches = np.repeat(batch_size, n_full_batches)
        # final_batch_len = N % batch_size
        # if final_batch_len:
        #     batches = np.append(batches, final_batch_len)
        ...
        for label_batch in labels_loader:
            ...
            # Expand dims of label for later concatenation
            label_batch = torch.unsqueeze(torch.unsqueeze(label_batch, 0), -1)
            label_batch = label_batch.to(self.device)

            # Initializing sequence from h_0 and z_tm1, could also start by
            # initializing x_tm1 and getting h_0 from there.
            # x_tm1 = torch.zeros(1, batch, self.x_dim).to(self.device)
            # h = self.deterministic_h(x_tm1)

            # Initialize h_0 [1, batch_size, dim_RNN_h]
            h = torch.zeros(1, batch_len, self.dim_RNN_h).to(self.device)
            # Initialize RNN hidden and cell state
            hidden_state = torch.zeros(self.rnn_h.num_layers, batch_len,
                                       self.rnn_h.hidden_size).to(self.device)
            cell_state = torch.zeros(self.rnn_h.num_layers, batch_len,
                                     self.rnn_h.hidden_size).to(self.device)
            # Initialize initial latent state z_tm1 (sample from standard Gaussian prior)
            z_tm1 = torch.randn(1, batch_len, self.z_dim).to(self.device)

            # Inner loop over time steps
            for t in range(seq_len):
                # Get next z from previous z and h
                z_mean, z_logvar = self.generation_z(h, z_tm1)
                # Sample from resulting distribution
                z = torch.normal(z_mean, torch.sqrt(torch.exp(z_logvar)))

                # Concatenate label with z (this is the conditioning step!)
                z_cat = torch.cat((z, label_batch), 2)
                # Generate x from h and z_cat
                x = self.generation_x(z_cat, h)
                ### Modification for masks as only input ###
                # x_distr = Bernoulli(probs=x)
                # x = x_distr.sample().int()
                ############################################
                # Append label to generated features, uncomment to keep generated label
                x[:,:,-1] = label_batch[:,:,-1]

                # Get next RNN state from generated x
                x_h = self.mlp_x_h(x)
                h, (hidden_state, cell_state) = self.rnn_h(x_h, (hidden_state, cell_state))

                z_tm1 = z

                x_seq.append(x)
            x_batched.append(torch.cat(x_seq))

        x_cat = torch.cat(x_batched, dim=1)

        # Split features from appended labels
        X_synth = x_cat[:,:,:-1]
        X_synth = X_synth.permute(1,-1,0)
