...
class KVAE(nn.Module):
    ...
    def __init__(self, x_dim, u_dim=1, a_dim=8, z_dim=4, activation='tanh',
    ...
    def build(self):
        ...
        if len(self.dense_x_a) == 0:
            ...
            for n in range(len(self.dense_x_a)):
                if n == 0:
                    dic_layers["linear" + str(n)] = nn.Linear(self.x_dim,
                                                              self.dense_x_a[n])
                else:
        ...
        self.iter_vae = list(self.mlp_x_a.parameters()) + list(self.inf_mean.parameters()) \
                        + list(self.inf_logvar.parameters()) + list(self.mlp_a_x.parameters()) + list(self.gen_mean.parameters())
    ...
    def generation_x(self, a):
        ...
        y_logvar = torch.log(0.1 * torch.ones_like(y_mean))

        # y = torch.exp(y_logvar)
        # y = y_mean
        y = self.reparameterization(y_mean, y_logvar)

        return y, y_mean, y_logvar
    ...
    def forward(self, x, u, compute_loss=False):
        ...
        if not self.use_smoothed_a:
            a_gen = a
        y, y_mean, y_logvar = self.generation_x(a_gen)

        # calculate loss
        if compute_loss:
            loss_tot, loss_vae, loss_lgssm = self.get_loss(x, y, y_mean, y_logvar, u_cat,
                                                           a, a_mean, a_logvar,
                                                           mu_smooth,
                                                           Sigma_smooth,
                                                           A_mix, B_mix, C_mix,
    ...
    def get_loss(self, x, y, y_mean, y_logvar, u, a, a_mean, a_logvar, mu_smooth, Sigma_smooth,
                 A, B, C, scale_reconstruction=1, seq_len=150, batch_size=32):

        # log p_{\theta}(x | a_hat), complex Gaussian
        # log_px_given_a = - torch.log(y) - x / y
        # log_px_given_a = - torch.square(x - y)
        const_log_pdf = (- 0.5 * np.log(2 * np.pi))
        log_px_given_a = const_log_pdf - 0.5 * y_logvar - torch.pow(x - y_mean, 2) / (
                    2 * torch.exp(y_logvar))
        # log_px_given_a = - torch.pow(x - y_mean, 2) / (2 * torch.exp(y_logvar))
    ...
    def generate_synth(self, labels, seq_len, det_gen=False, batch_size=64):
        ...
        for label_batch in labels_loader:
            ...
            label_batch = label_batch.view(batch_len, self.u_dim)
            ...
            x_seq, _, _ = self.generation_x(torch.stack(a_seq, dim=1).squeeze())
            x_batched.append(x_seq)
