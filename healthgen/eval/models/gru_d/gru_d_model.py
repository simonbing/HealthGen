...
from healthgen.eval.models.gru_d.gru_d_layer import GRUD_cell


def grud_model_old(seed, input_size, hidden_size, output_size, num_layers=1, x_mean=0, \
    ...
    for i in range(
            num_layers - 1):  # subtract 1 bc last layer needs to be called with different params
        layer = GRUD_cell(seed=seed, input_size=input_size, hidden_size=hidden_size,
                          output_size=input_size, dropout=dropout,
                          dropout_type=dropout_type, x_mean=x_mean,
                          num_layers=num_layers, return_hidden=True)
    ...
    layer = GRUD_cell(seed=seed, input_size=input_size, hidden_size=hidden_size,
                      ...
                      dropout_type=dropout_type, x_mean=x_mean,
                      num_layers=num_layers, return_hidden=False)
    layer_list.append(layer)
...
class gru_d_model(torch.nn.Module):
