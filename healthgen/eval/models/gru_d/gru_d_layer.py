...
class GRUD_cell(torch.nn.Module):
    ...
    def __init__(self, seed, input_size, hidden_size, output_size, num_layers=1,
                 ...
                 dropout_type='mloss', dropout=0, return_hidden=False):
    ...
    def forward(self, input):
        ...
        output_tensor = torch.empty(
            [X.size()[0], X.size()[2], self.output_size], dtype=X.dtype,
            device=device)
        ...
        for timestep in range(X.size()[2]):
            ...
            # EXPERIMENTAL: omiiting squeeze to be able to handle single features
            ...
            x = m * x + (1 - m) * (
                        gamma_x * x_last_obsv + (1 - gamma_x) * x_mean)
            ...
            if self.dropout == 0:
                ...
                h_tilde = torch.tanh(
                    self.w_xh(x) + self.w_hh(r * h) + self.w_mh(m))
